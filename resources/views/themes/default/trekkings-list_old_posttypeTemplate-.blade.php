    ...
    <div class="uk_header" uk-scrollspy-class>
        <div class=" uk_header_image uk-position-relative">
            <img src="@if($data->banner){{ asset('uploads/medium/' . $data->banner) }}@else{{asset('themes-assets/images/default/default-banner.jpg')}}@endif"
                class="uk-image" alt="{{$data->post_type}}" loading="eager">
        </div>
...
<section class="uk-section bg-white uk-position-relative">
    ...
    @if($trekkings->count() > 0)
        <div class="uk-container">
            <ul class=" uk-child-width-1-2@s uk-child-width-1-3@l uk-text-left uk-flex uk-flex-center"
            ...
            <ul class=" pagination uk-flex-center uk-margin-large-top">
                {!! $trekkings->links('themes.default.common.pagination') !!}
            </ul>
        </div>
    @else
        <div class="uk-container">
