                ...
                <div class="table-responsive mhn20 mvn15">
                      <table class="table admin-form table-striped dataTable" id="datatable3">
                        ...
                        @if(count($inquiry) > 0)
                           ...
                           <tr class="bg-light">
                            <td class="">{{$key+=1}}</td>                               
                            <td class="">{{ ucfirst($row->name) }} <br>{{ ($row->email) }}<br>{{$row->number}}<br>{{$row->country}}</td>
                            <td class="">{{ ucfirst($row->title) }}</td>                 
                            <td class="">{{ ($row->trip_start_date) }}</td>
                            <td class=""><textarea>{!!$row->review!!}</textarea></td> 
                            ...
                            <td class="text-left">
                              <form action="{{ route('trip-inquiry.destroy', $row->id) }}" method="POST">
                                {{ method_field('DELETE') }}
                                {{ csrf_field() }}
                                <button onclick="return confirm('Are you sure you want to delete this item?');" class="fa fa-trash form-control" style="color:red;"></button>
                              </form>
                            </td>
                        </tr>
                        @endforeach
